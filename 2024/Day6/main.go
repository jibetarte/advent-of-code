...
func part2(parsed_input [][]rune) int {
	...
	dir := "N"
	pos_x, pos_y := findPlayer(parsed_input)
	parsed_input[pos_y][pos_x] = rune('.')
	result = advancePlayerPart2(parsed_input, pos_x, pos_y, dir, 0)
	return result
...
func advancePlayerPart2(parsed_input [][]rune, pos_x int, pos_y int, dir string, count int) int {
	...
	case "N":
		...
		parsed_input[pos_y-1][pos_x] = rune('#')
		is_loop := searchLoop(parsed_input, pos_x, pos_y, dir, pos_x, pos_y, true)
		parsed_input[pos_y-1][pos_x] = rune('.')
	...
	case "E":
		...
		parsed_input[pos_y][pos_x+1] = rune('#')
		is_loop := searchLoop(parsed_input, pos_x, pos_y, dir, pos_x, pos_y, true)
		parsed_input[pos_y][pos_x+1] = rune('.')
...
func searchLoop(board [][]rune, pos_x int, pos_y int, dir string, initial_pos_x int, initial_pos_y int, is_first_time bool) bool {
	...
	case "N":
		...
		if initial_pos_x == pos_x && initial_pos_y == pos_y && !is_first_time && string(board[pos_y-1][pos_x]) == "#" {
			return true
		}
		if is_first_time {
			is_first_time = false
	...
	case "E":
		...
		if initial_pos_x == pos_x && initial_pos_y == pos_y && !is_first_time && string(board[pos_y][pos_x+1]) == "#" {
			return true
		}
		if is_first_time {
			is_first_time = false
